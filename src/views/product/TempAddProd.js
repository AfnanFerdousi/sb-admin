...
const CommonAddProduct = ({ productType }) => {
    ...
    //console.log('CommonAddProduct ~ categoryID', categoryID)
    const catgories = watch('Categories')
    //console.log('CommonAddProduct ~ catgories', catgories)
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const categoryIDs = categoryID.map((item) => item.value)
            ...
            bodyFormData.append('ParentCategoryID', selectedCategory ? selectedCategory : 0)
    ...
    useEffect(() => {
        const loadCategory = async () => {
            const response = await axiosJWT.get(
                GET_CATEGORIES_BY_SUBJECT.replace('[[productType]]', productType),
                {},
                {
                    headers: {
                        Authorization: `${token}`,
    ...
    const allCategoryOptions = categories?.map((category) => {
        return {
    ...
    useEffect(() => {
        const filteredCategory = categories.filter((category) => category?.ParentCategoryID === selectedCategory?.value)
                                            ...
                                            {productType == "ACADEMIC_BOOK" ? (
                                                <>
                                                    <CFormInput
                                                        ...
                                                        floatingLabel="Selected Categories"
                                                        className={'my-2'}
