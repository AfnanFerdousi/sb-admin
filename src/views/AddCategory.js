  ...
  const onEdit = async (data) => {
    try {
      ...
      if (data?.NewCategoryBanner[0]) {
        ...
        const imageRes = data?.NewCategoryBanner[0] && await axiosJWT
          .post(FILE_UPLOAD,
            imageBodyFormData, {
            headers: {
              'Content-Type': 'multipart/form-data',
      ...
      const res = await trackPromise(axiosJWT.post(`${baseURL}/api/v1/admin/category/edit`, bodyFormData, {
        headers: {
          'Content-Type': 'multipart/form-data',
          Authorization: `${token}`,
        },
